Peer-review:
· Предоставлено решение для всех пунктов домашнего задания №1.
· Каждое задание отделено от предыдущего соответствующим комментарием. Даны комментарии о том, что делает код и о том,
какие результаты ожидаются. До момента запуска кода вопросов не возникает, понятно какая часть к чему относится.
· По содержанию все сделано в соответсвии с заданиеем: 1) создаются таблицы; 2) импортируются данные в таблицу;
3)создается внешний ключ; 4) выполняется запрос, указано количество, выведенных значений.
· Есть комментарии по поводу этапа подготовки данных: не было сказано про удаление скрытых листов из исходных файлов перед 
конвертированием их в формат csv, из-за чего может возникать ошибка при создании файла для загрузки. В файле bond_description
были скрытые столбцы, которые не были созданы в таблице bond_description, хотя в задании говорилось о сохранении структуры файла.
Но в целом все описано, в том числе какие форматы и в какой строчке необходимо изменить, и используются подходящие форматы данных.
Мне чтобы загрузить поля с датами нужно было изменить формат даты с ДД.ММ.ГГГГ на ММ.ДД.ГГГГ
В таблице quotes "TIME" integer NOT NULL можно было тоже перевести в формат даты и использовать тип поля даты. + поле "TIME" и "ID"
не уникальны, поэтому не могут быть PRIMARY KEY. Для формата real "," нужно заменить на ".".
